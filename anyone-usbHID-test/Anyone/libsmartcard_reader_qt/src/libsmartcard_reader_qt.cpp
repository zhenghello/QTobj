...
bool libsmartcard_reader_qt::smartcard_read(MF &pmf, UINT8 &invalid_card_info, UINT16 &cpu_card_error_code, bool &timeout_flag, bool &usb_open_flag, UINT8* pcard_version_id, UINT8 which_channel /* = 0x00 */, int timeout /* = 20000 */, UINT8 oem /* = */ )
{
	...
	action_over = false;
	if(this->puserhid->hidOpen())//打开usb口
	{
		if(this->cmd_oemset(which_channel, oem))
		{
			...
			if(this->cmd_open(which_channel))
			{
				...
				while(!action_over)
				{
					//while(current_time > QDateTime::currentMSecsSinceEpoch()) //等待10s，设备数据上传完全后再处理
					//{
					//	;
					//}
					...
					if(puserhid->hidParse(pbuff, len, cmd_type, timeout))
					{
						...
						{
						case SMARTCARDUPLOADBEGIN: //上传卡数据开始
							break;
						case SMARTCARDUPLAODINFO: //上传卡数据
							{
								...
								else
								{
									...
									{
										if(df_temp == pmf.DFList.at(i)->P2) //DF的位置
										{
											df_exist_index = i;
											break;
										}
									}
									if(df_exist_index == -1) //DF还没添加,先添加DF
									{
							...
							break;
						case SMARTCARDUPLOADEND: //上传卡数据完成，标识读卡成功完成
							{
								action_over = true;
								b = true;
							}
							break;
						case SMARTCARDUPLOADINVALIDCARD: //读卡异常，无效卡
							{
							...
							break;
						case SMARTCARDUPLOADCARDINFO: //上传卡版本以及ID
							{
								memcpy(pcard_version_id, pbuff, len);
							}
							break;
						case SMARTCARDUPLOADCARDERRORCODE: //读卡异常，上传CPU卡操作错误信息
							{
...
bool libsmartcard_reader_qt::smartcard_write(unsigned char df_p2, unsigned char ef_p2, unsigned short offset, unsigned short len, char* pbuff, int timeout /* = 1000 */, UINT8 which_channel /* = 0x00 */, UINT8 oem /* = */ )
{
	...
	if(puserhid->hidOpen())
	{
		if(this->cmd_oemset(which_channel, oem))
		{
			b = this->cmd_write(df_p2, ef_p2, offset, len, pbuff, timeout, which_channel); //写卡
		}
		puserhid->hidClose();
		return b;
	}
...
bool libsmartcard_reader_qt::smartcard_create(MF &pmf, UINT8 which_channel /* = 0x00 */, UINT8 oem /* = */ )
{
	...
	if(puserhid->hidOpen())
	{
		...
		for(int i = 0;i < mf_ef_length;i++) //创建MF下的EF
		{
			QByteArray efByteArray = pmf.EFList.at(i)->P2.toAscii();
		...
		{
			QByteArray dfByteArray = pmf.DFList.at(i)->P2.toAscii();
			UINT8 df_dif = dfByteArray.at(1) - '0';
			int df_ef_length = pmf.DFList.at(i)->EFList.length();
			for(int j = 0;j < df_ef_length;j++)
			{
				QByteArray efByteArray = pmf.DFList.at(i)->EFList.at(j)->P2.toAscii();
				UINT8 ef_dir = efByteArray.at(1) - '0';
