...
class Fdebug : public QWidget
{
    Q_OBJECT
public:
    explicit Fdebug(QString firstShow = "Hello World!", bool hideKey = true , QWidget *parent = 0);
    ~Fdebug();
    // 内部参数
    /* 懒汉式->单例模式 Fdebug::myDebug()->*/
    static Fdebug *myDebug()      // 用于调试界面
    {
        static Fdebug *my_debug;
        if(my_debug == NULL) //判断是否第一次调用
        {
            my_debug = new Fdebug("myDebug");
        }
        return my_debug;
    }
    static Fdebug *myqDebug()      // 用于调试界面
    {
        static Fdebug *my_qdebug;
        if(my_qdebug == NULL) //判断是否第一次调用
        {
            my_qdebug = new Fdebug("myqDebug");
            qInstallMessageHandler(qDebugOutput);// 关联
        }
        return my_qdebug;
    }
    static Fdebug *myShow()       // 用于主显示
    {
        static Fdebug *my_show;
        if(my_show == NULL)
            my_show = new Fdebug("myShow");
        return my_show;
    }
    static Fdebug *myTest()       // 用于测试
    {
        static Fdebug *my_test;
        if(my_test == NULL)
            my_test = new Fdebug("myTest");
        return my_test;
    }
    // 显示相关
...
private:
    ...
    QLineEdit *e_debug1;
    QTextEdit *e_debug2;
    QPushButton *b_debug1;
    QPushButton *b_debug2;
    QPushButton *b_clear;
