...
// **************************************************************************************************************************************
bool hid_is_connect(HANDLE hDevice, USHORT vid, USHORT pid)
{
    // 查询hid设备属性
 ...
 * 返回：读取成功/失败
 */
bool read_hid(HANDLE hDevice,char *buff,ulong max_len,ulong *real_len)
{
    BOOL                    bResult;
    OVERLAPPED              HidOverlapped;  // 阻塞事件
    HANDLE                  ReportEvent;    // 阻塞读取的延时事件HANDLE
    // 1.判断输入
    if((hDevice == NULL) || (buff == NULL) ||(max_len<(READ_PACK_LEN+1)) ||(real_len==NULL))return FALSE;
    // 2.为捕获数据创建一个新的事件
    ...
    {
        // qDebug("read fail!error_code=%d",GetLastError());
        return FALSE;
    }
}
/**********************************************************
...
* 输出参数：写入成功/失败
**********************************************************/
bool write_hid(HANDLE hDevice,char buff[WRITE_PACK_LEN+1])
{
    ...
    {
        // qDebug("write fail!error_code=%d",GetLastError());
        return FALSE;
    }
}
