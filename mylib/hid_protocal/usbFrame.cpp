...
usbFrame::usbFrame(int VID, int PID, char PORT)
{
    phid = 0;
    vid  = VID;
    pid  = PID;
...
// 判断hid设备是否连接
bool usbFrame::hidIsConnect(void)
{
    return hid_is_connect(phid,vid,pid);
}
// 打开hid设备
bool usbFrame::hidOpen()
{
    bool b;
...
// 重定义的名字
bool usbFrame::frameTx(const QVector<char> send)
{
    return hidSend(send);
}
// 发送usb数据->注意，这个buff是会被修改的
bool usbFrame::hidSend(const QVector<char> send)
{
    bool b = false;
    // 输入错误
    if( send.size()==0)
