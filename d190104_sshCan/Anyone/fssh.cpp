...
{
    return sftpChannel->downloadFile(remotePath, localPath, QSsh::SftpOverwriteExisting);
}
// ******************************************** 外部函数 **************************************************************************************** end
...
{
    qDebug() << "Ssh:onSshError " << error << sshErrorToString(error);
    sshConnection->deleteLater();
    sshConnection   = NULL;
    sshShell        = NULL;
    sftpChannel     = NULL;
...
{
    emit downloadFileFinished(job, error);
}
